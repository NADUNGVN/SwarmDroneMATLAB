...
for frame=1:F
    ...
    if ~barrierClosed && prepareDue(frame)
        ...
        for node=reshape(affected(affected~=initiator),1,[])
            if ~currentPrepared(node), continue; end
...
O=struct('version','RECEIVER-LIFTED-CLOSURE-KERNEL-v1', ...
...
attemptBound=preparePolicy.opportunityCount+ ...
    preparePolicy.opportunityCount*max(0,S-1)+S+ ...
...
byteBound=preparePolicy.opportunityCount*M.prepareBytes+ ...
    ...
    S*M.config.revokeBytes+claimOpportunities*S*M.config.claimBytes+ ...
