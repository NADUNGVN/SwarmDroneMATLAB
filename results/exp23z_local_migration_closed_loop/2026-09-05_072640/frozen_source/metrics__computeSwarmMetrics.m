...
if isfield(out,'desiredOffsets')
    desiredOffsets=double(out.desiredOffsets);
    if ~isequal(size(desiredOffsets),[K N 3])
        error('computeSwarmMetrics: malformed desiredOffsets log.');
    end
else
    desiredOffsets=repmat(reshape(offsets,[1 N 3]),K,1,1);
end
desiredPos = leaderPos + desiredOffsets;
