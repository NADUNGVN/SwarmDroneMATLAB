function conflict=buildSenderConflictGraph(neighbor,interference,detectability)
...
if nargin<3||isempty(detectability)
    detectability=false(N);
elseif ~isequal(size(detectability),[N N])
    error('buildSenderConflictGraph: detectability must match graphs.');
end
detectable=interference|neighbor|logical(detectability);
conflict=false(N);
for a=1:N
