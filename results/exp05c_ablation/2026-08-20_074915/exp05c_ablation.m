...
expRun = startExperiment('exp05c_ablation');
...
%
% Only variant A4 maintains accepted-state memory. ackSyncMissCount
% counts occasions where the transmitter could not match a receiver
% acceptance to a pending attempt; it should be 0. Nothing previously
% inspected it, so it is recorded here.
% ------------------------------------------------------------
...
%% ------------------------------------------------------------
% Per-scenario contribution numbers
%
% These are headline results, so they are stored as arrays rather
% than loop-local scalars.
% ------------------------------------------------------------

gainA2 = nan(1,nScenario);
gainA3 = nan(1,nScenario);
gainA4 = nan(1,nScenario);

rateA2 = nan(1,nScenario);
rateA3 = nan(1,nScenario);
rateA4 = nan(1,nScenario);
...
% inflate that link's trigger rate and contaminate the A3 -> A4
% contribution reported above.
% ============================================================
...
%
% One row per (seed, ablation variant, network scenario).
% ============================================================
...
T = tidyFromArray( ...
    ...
    {'seed','variant','scenario'}, ...
    {1:numSeeds, variantNames, scenarioNames});
