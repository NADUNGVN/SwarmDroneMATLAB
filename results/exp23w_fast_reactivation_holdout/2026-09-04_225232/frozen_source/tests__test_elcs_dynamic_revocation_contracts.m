...
fastC=C; fastC.witnessConfirmedEarlyReactivation=true;
fastC.migrationUnionGraphCertified=true;
F=simulateElcsWitnessScheduling(fastC,T);
assert(F.debug.reacquired(15,3) && F.debug.active(15,3) && ...
    F.reacquisitionLatencyFrames(3)==7 && ...
    F.scheduledCollisionFrames==0);
fastBlocked=simulateElcsWitnessScheduling(fastC,blocked);
assert(fastBlocked.finalSuppressed(3) && ...
    fastBlocked.reacquisitionCount==0 && ...
    fastBlocked.scheduledCollisionFrames==0);
fprintf(['    ok   union-graph witness receipts permit safe early reuse while ' ...
    'missing receipts remain silent\n']);
checks=checks+1;
