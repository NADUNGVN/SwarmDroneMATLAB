...
The queue-compatible rule scans `W` independent access draws in minislot
order. A node transmits at the first draw no larger than `p`, then becomes
ineligible for the remaining fallback minislots of that frame. Consequently,
each node makes at most one fallback attempt per frame and cannot spend two
opportunities on the same latest-generated-wins state. Its probability of an
attempt somewhere in the fallback window remains

`a_W = 1-(1-p)^W`.

For `m` independent fallback contenders and conditional recipient success
probability at least `q>0`, the probability that a particular node selects
minislot `w` and no other contender selects that slot is

`p(1-p)^(w-1) [1-p(1-p)^(w-1)]^(m-1)`.

Therefore its per-frame success probability is

`r_(m,W) = q sum_(w=1)^W p(1-p)^(w-1)
                 [1-p(1-p)^(w-1)]^(m-1)`

and admits the conservative lower bound

`r_(m,W) >= q [1-(1-p)^W] (1-p)^(m-1)`.

For `p=1/m`, the expected number of fallback frames to success is at most the
reciprocal of this positive bound. This is a service bound under the declared
full-contention/i.i.d. model, not a deterministic delivery guarantee.
...
8. every control, fallback and scheduled attempt is separately charged and
   each node makes at most one fallback attempt per frame;
