...
switch lower(char(variant))
    ...
    case 'v3'
        ...
        experimentName='exp22c_elcs_kernel_activation';
    case 'v4'
        R=exp22eElcsKernelRegistry();
        experimentName='exp22e_elcs_fallback_repair';
    otherwise
...
if isfield(R,'fallbackAttemptCapPerNodeFrame')
    capped=all(T.MAX_FALLBACK_ATTEMPTS_PER_NODE_FRAME<= ...
        R.fallbackAttemptCapPerNodeFrame);
    add('fallback_attempt_cap',capped,sprintf( ...
        'maximum observed per node/frame %.0f (cap %d)', ...
        max(T.MAX_FALLBACK_ATTEMPTS_PER_NODE_FRAME), ...
        R.fallbackAttemptCapPerNodeFrame));
end
...
if startsWith(R.version,'EXP22E')
    ...
    registry='utils/exp22eElcsKernelRegistry.m';
elseif startsWith(R.version,'EXP22C')
    plan='docs/EXP22C_ELCS_KERNEL_ACTIVATION_PLAN.md';
    registry='utils/exp22cElcsKernelRegistry.m';
elseif startsWith(R.version,'EXP22B')
...
files={'docs/EXP22_EDGE_LEASE_CANDIDATE_DESIGN.md',plan, ...
    ...
    'utils/exp22ElcsKernelRegistry.m', ...
    'utils/exp22bElcsKernelRegistry.m', ...
    'utils/exp22cElcsKernelRegistry.m', ...
    'utils/exp22eElcsKernelRegistry.m', ...
