...
%
% The per-scenario frontiers above let every policy be retuned for
% each network. That is not how the method is meant to be used.
%
% The actual claim is that ONE parameter set copes with ALL network
% conditions. The fair counterpart is therefore ONE fixed rate used
% across all conditions. This block compares:
%
%   cost  = mean communication rate over the scenarios
%   risk  = WORST formation RMSE over the scenarios
%
% A configuration is only useful if its worst case is acceptable, so
% the worst case is the right error statistic here.
% ============================================================

fprintf('\n');
fprintf('============================================================\n');
fprintf('One configuration across all network conditions\n');
fprintf('============================================================\n\n');

meanRateAcross  = mean(meanTXRATE, 3);          % family x point
worstRMSEAcross = max(meanRMSE,  [], 3);        % family x point
meanRMSEAcross  = mean(meanRMSE,   3);          % family x point

fprintf('%-12s %-9s %10s %12s %12s\n', ...
    'Family','Setting','MeanRate','WorstRMSE','MeanRMSE');
...
for iF = 1:nFamily
    ...
    for iP = 1:nPoint
        ...
        fprintf('%-12s %-9s %10.2f %12.4f %12.4f\n', ...
            familyNames{iF}, ...
            setting, ...
            meanRateAcross(iF,iP), ...
            worstRMSEAcross(iF,iP), ...
            meanRMSEAcross(iF,iP));
...
%% ------------------------------------------------------------
% Dominance on the single-configuration plane
% ------------------------------------------------------------

fprintf('Margin on the single-configuration plane\n');
fprintf('(baseline worst-case RMSE at equal mean rate) - (Full-AoI)\n\n');

singleMarginPeriodic = nan(nPoint,1);
singleMarginEvent    = nan(nPoint,1);

fprintf('  %10s %12s %14s %16s\n', ...
    'MeanRate','AoI Worst','vs Periodic','vs State-event');

for iP = 1:nPoint

    rate = meanRateAcross(3,iP);

    singleMarginPeriodic(iP) = ...
        localInterp(meanRateAcross(1,:), worstRMSEAcross(1,:), rate) ...
        - worstRMSEAcross(3,iP);

    singleMarginEvent(iP) = ...
        localInterp(meanRateAcross(2,:), worstRMSEAcross(2,:), rate) ...
        - worstRMSEAcross(3,iP);

    fprintf('  %10.2f %12.4f %+14.4f %+16.4f\n', ...
        rate, ...
        worstRMSEAcross(3,iP), ...
        singleMarginPeriodic(iP), ...
        singleMarginEvent(iP));
...
fprintf('\n  Full-AoI better at %d/%d comparable points vs Periodic, %d/%d vs State-event\n', ...
    sum(singleMarginPeriodic > 0), sum(~isnan(singleMarginPeriodic)), ...
    sum(singleMarginEvent    > 0), sum(~isnan(singleMarginEvent)));


%% ------------------------------------------------------------
% Rate adaptivity
%
% How much does each policy change its transmission rate between the
% cleanest and the harshest network, holding its configuration fixed?
% This is the behaviour the method is designed to exhibit.
% ------------------------------------------------------------

fprintf('\n');
fprintf('Rate adaptation from %s to %s (same configuration)\n\n', ...
    scenarioNames{1}, scenarioNames{end});

fprintf('%-12s %-9s %10s %10s %10s\n', ...
    'Family','Setting','Clean[Hz]','Stress[Hz]','Ratio');

adaptRatio = zeros(nFamily,nPoint);

for iF = 1:nFamily

    for iP = 1:nPoint

        rClean  = meanTXRATE(iF,iP,1);
        rStress = meanTXRATE(iF,iP,nScenario);

        adaptRatio(iF,iP) = rStress / max(rClean,eps);
        ...
        fprintf('%-12s %-9s %10.2f %10.2f %10.2f\n', ...
            familyNames{iF}, setting, rClean, rStress, adaptRatio(iF,iP));
...
figure('Name','EXP05D single configuration across all networks');

hold on; grid on;

for iF = 1:nFamily

    plot(meanRateAcross(iF,:), worstRMSEAcross(iF,:), markers{iF}, ...
...
xlabel('Mean communication rate per channel across networks [Hz]');
ylabel('Worst-case formation RMSE across networks [m]');
title('One configuration for all network conditions');
legend(familyNames, 'Location', 'northeast');
