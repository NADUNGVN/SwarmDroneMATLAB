...
row.ELCSW_REVOKE_ATTEMPTS=K.revokeAttempts;
...
row.ELCSW_REVOKE_BYTES=K.revokeBytes;
row.ELCSW_SELF_REVOCATIONS=K.selfRevocationCount;
row.ELCSW_REACQUISITIONS=K.reacquisitionCount;
row.ELCSW_FINAL_SUPPRESSED_NODES=nnz(K.finalSuppressed);
row.ELCSW_WITNESS_CONFIRMED_REACTIVATION= ...
    K.witnessConfirmedEarlyReactivation;
