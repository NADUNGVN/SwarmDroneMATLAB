function exp14c_mac_aware_development()
...
cells=unique(S.scenarioLabel,'stable');
...
for k=1:numel(cells)
    ...
    for j=1:numel(arms)
        Q=S(S.scenarioLabel==cells(k) & S.arm==arms(j),:);
        scatter(ax,Q.meanOfferedUtil,Q.meanRMSE,70,colors(j,:),'filled', ...
            'DisplayName',Q.methodLabel);
        if Q.safeFailures>0
...
files={'docs/EXP14C_MAC_AWARE_DEVELOPMENT_PLAN.md', ...
    ...
    'utils/applyExp14CArm.m','network/macAwarePolicyConfig.m', ...
    'network/macAwareLoadGuard.m','network/adaptiveStandaloneAckDecision.m', ...
    'network/sharedMediumFeedbackMode.m','network/enqueueAckSummary.m', ...
