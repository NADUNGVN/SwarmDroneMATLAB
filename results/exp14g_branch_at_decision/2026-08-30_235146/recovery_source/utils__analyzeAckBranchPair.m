...
requestedEnd=A.time+cfg.shared.ackBranchReplay.localHorizon;
endIndex=find(actual.t>=requestedEnd-1e-12,1,'first');
if isempty(endIndex) || ~isequal(actual.t,shadow.t)
    error('analyzeAckBranchPair: local outcome horizon is invalid.');
end
windowEnd=actual.t(endIndex);
...
ackCost=actual.ackAirtimeLog(endIndex)-shadow.ackAirtimeLog(endIndex);
dataSaved=shadow.dataAirtimeLog(endIndex)-actual.dataAirtimeLog(endIndex);
netBenefit=(shadow.dataAirtimeLog(endIndex)+shadow.ackAirtimeLog(endIndex)) ...
    -(actual.dataAirtimeLog(endIndex)+actual.ackAirtimeLog(endIndex));
...
row.dataAttemptsSaved=shadow.dataAttemptLog(endIndex) ...
    -actual.dataAttemptLog(endIndex);
row.collisionsSaved=shadow.collisionLog(endIndex) ...
    -actual.collisionLog(endIndex);
