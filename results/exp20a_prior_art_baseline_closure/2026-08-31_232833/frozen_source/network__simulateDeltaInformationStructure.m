...
for slot=1:L
    ...
    switch arm
        case 'delta-public'
            [tx,psi(:,1),collisionSteps(:,1),txProbability] = ...
            ...
            txProbability=txProbability; %#ok<NASGU>
        ...
        case 'zero-wait'
            tx=sourcePending & (D.accessU(slot,:)'<=1/N);

        case 'max-age-first'
            eligible=find(sourcePending);
            if ~isempty(eligible)
                [~,k]=max(gatewayAoII(eligible));
                tx(eligible(k))=true;
            end

        case 'round-robin'
            owner=mod(slot-1,N)+1;
            tx(owner)=sourcePending(owner);
    end
...
if phase==0
    ...
    localAge=max(slot-pendingSince,0);
