...
distributedMode=any(strcmp(S.config.mode, ...
    {'local-static-tdma','distributed-reservation'}));
if strcmp(S.config.mode,'continuous-local-static-tdma')
    error(['selectScheduledService: continuous scheduling must be ' ...
        'dispatched by advanceSharedMedium event timing.']);
end
if distributedMode
    [S,candidates]=distributedReservationStep( ...
...
elseif ~S.config.enabled || backgroundActive || ~isempty(net.active)
    return;
else
    eligible=find(hasFrame(:));
end
...
switch S.config.mode
    case {'local-static-tdma','distributed-reservation'}
        % Candidate selection and all coordination state were evaluated by
        % distributedReservationStep above using only local state and the
        % current absolute trace index.
