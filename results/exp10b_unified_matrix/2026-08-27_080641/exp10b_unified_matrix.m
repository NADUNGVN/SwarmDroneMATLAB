...
for c = 1:nCell
    for s = 1:numSeeds
        ...
        fwd = arrayfun(@(iM) T.FWDHASH(rowOf(s,iM,c)), 1:nMethod);
...
for c = 1:nCell
    ...
    for iM = 1:nMethod
        ...
        U.point{rowIdx}     = pt.id;
        U.plant{rowIdx}     = pt.plant;
        U.N(rowIdx)         = pt.N;
        U.topology{rowIdx}  = pt.topology;
        U.scenario{rowIdx}  = sc.names{cellScen(c)};
        U.method{rowIdx}    = methodNames{iM};

        U.RMSEmean(rowIdx) = mRMSE(iM,c);
        U.RMSEstd(rowIdx)  = sRMSE(iM,c);
        U.minSep(rowIdx)   = mMINSEP(iM,c);

        U.safeFailCount(rowIdx)   = safeUnsafe(iM,c);
        U.safeEligibleCount(rowIdx) = safeElig(iM,c);
        U.safeFailRate(rowIdx)    = safeRate(iM,c);
        U.safeRule{rowIdx}        = safeRule{iM,c};

        U.DATA(rowIdx)      = mDATA(iM,c);
        U.ACK(rowIdx)       = mACK(iM,c);
        U.Total_w010(rowIdx) = mT010(iM,c);
        U.Total_w025(rowIdx) = mT025(iM,c);
        U.Total_w050(rowIdx) = mT050(iM,c);
        U.airtime(rowIdx)   = mAIR(iM,c);
        U.broadcast(rowIdx) = mBCST(iM,c);

        U.AoI(rowIdx)          = mAOI(iM,c);
        U.estimatedAoI(rowIdx) = mEAOI(iM,c);

        U.saturation(rowIdx) = mSAT(iM,c);
        U.effort(rowIdx)     = mEFF(iM,c);
        U.estError(rowIdx)   = mESTERR(iM,c);

        U.diverged(rowIdx)  = nDIV(iM,c);
        U.evaluable(rowIdx) = nEVAL(iM,c);
        U.connected(rowIdx) = nCONN(iM,c);

        for v = 1:nVariant
            U.(sprintf('dominated_%s', localSafeName(variantNames{v})))(rowIdx) = ...
                double(dominated(iM,c,v));
        end

        U.sourceExperiment{rowIdx} = pt.source;
        U.sourceTag{rowIdx}        = pt.sourceTag;
