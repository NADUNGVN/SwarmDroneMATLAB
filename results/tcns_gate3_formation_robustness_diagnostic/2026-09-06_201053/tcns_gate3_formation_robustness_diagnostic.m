...
for s = 1:numel(seeds)
    ...
    theoretical = tcnsFiniteHorizonFormationBound(cfg,D.bound(idx,:));
    ...
    row.maxDisturbanceBound_mps2 = max(D.bound(idx,:),[],'all');
    row.maxDisturbanceViolation_mps2 = max( ...
        D.actualNorm(idx,:)-D.bound(idx,:),[],'all');
...
summary.interpretation = [ ...
    'The theorem bounds communication-induced degradation relative to a ' ...
    'perfect-current-information continuation sharing the state at 8 s. ' ...
    'Coverage does not establish policy superiority.'];
...
row = struct('seed',NaN,'scenario',"",'method',"", ...
    ...
    'maxFinitePositionBound_m',NaN,'maxStructuredPositionUUB_m',NaN, ...
