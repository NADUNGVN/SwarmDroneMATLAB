...
for frame=reshape(used,1,[])
    ...
    if isfield(K.debug,'revokeTx')
        revokeNodes=find(K.debug.revokeTx(frame,:));
        if any(K.debug.claimTx(frame,revokeNodes))
            error(['buildElcsWitnessContinuousSchedule: CLAIM and REVOKE ' ...
                'share one node minislot.']);
        end
        for node=reshape(revokeNodes,1,[])
            tk=fs+(node-1)*Sclaim;
            appendControl(frame,node,node,'revoke',tk, ...
                C.coherenceRevokeBytes, ...
                squeeze(K.debug.revokeSuccess(frame,:,:)), ...
                squeeze(K.debug.revokeErasure(frame,:,:)), ...
                squeeze(K.debug.revokeCollision(frame,:,:)));
        end
    end
...
Q.physicalFrameCount=numel(used);
Q.physicalFinalAllCertified=false;
Q.physicalUncertifiedNodeFrames=0;
Q.physicalRetryClaimAttempts=0;
Q.physicalFallbackCollisionFrames=0;
if ~isempty(used)
    Q.physicalFinalAllCertified=all(K.debug.active(used(end),:));
    Q.physicalUncertifiedNodeFrames=nnz(~K.debug.active(used,:));
    periodicMask=mod(used-1,C.renewalPeriodFrames)==0;
    Q.physicalRetryClaimAttempts=nnz( ...
        K.debug.forceClaim(used,:) & ~periodicMask);
    fallbackRows=Q.dataKind=="fallback" & any(Q.dataCollisionMask,2);
    Q.physicalFallbackCollisionFrames=numel(unique( ...
        Q.dataFrame(fallbackRows)));
end
