...
Drevoke=0;
hasRevocation=isfield(C,'dynamicRevocationEnabled') && ...
    logical(C.dynamicRevocationEnabled) && ...
    isfield(C,'coherenceRevokeBytes') && ...
    isfield(T,'revokeDeliveryU') && isfield(T,'selfRevoke');
if hasRevocation, Drevoke=8*C.coherenceRevokeBytes/C.phyRateBps; end
...
claimHits=0; revokeHits=0; responseHits=0; scheduledHits=0; fallbackHits=0;
for frame=1:F
    ...
    for sender=1:N
        ...
        if hasRevocation && T.selfRevoke(frame,sender) && ...
                occupied(actual,Drevoke)
            T.revokeDeliveryU(frame,:,sender)=0;
            revokeHits=revokeHits+1;
        end
...
B=struct('version','ELCS-W-SHARED-BACKGROUND-OVERLAY-v1', ...
    ...
    'totalPotentialAttemptHits',claimHits+revokeHits+responseHits+ ...
        scheduledHits+fallbackHits);
B.hashExact=realizationHash([backgroundLoad;trace.hashExact; ...
    claimHits;revokeHits;responseHits;scheduledHits;fallbackHits;T.hashExact]);
