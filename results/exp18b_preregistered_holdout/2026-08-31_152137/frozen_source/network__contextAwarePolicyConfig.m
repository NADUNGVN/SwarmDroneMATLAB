...
policy=setDefault(policy,'routeRule','marginal-value');
...
policy.routeRule=lower(strtrim(char(policy.routeRule)));
if ~any(strcmp(policy.routeRule,{'marginal-value','capacity-gated-mac'}))
    error(['contextAwarePolicyConfig: routeRule must be marginal-value or ' ...
        'capacity-gated-mac.']);
end
