...
%   'periodic-feedback' fixed-period state broadcast with causal feedback
...
%   'context-aware-broadcast' feasibility-first access and ACK marginal value
...
cfg.mac = mac;
if isfield(cfg.mac,'interferenceTransitions')
    interferenceMatrixAtTime(cfg.mac.interferenceMatrix, ...
        cfg.mac.interferenceTransitions,0);
end
...
for k = 1:K
    ...
    if k < K
        for sender = 1:N
            ...
            switch method
                case {'periodic','periodic-feedback'}
    ...
    currentOffsets=cfg.swarm.offsets;
    if isfield(cfg.swarm,'offsetTransitions')
        currentOffsets=formationOffsetsAtTime(cfg.swarm.offsets, ...
            cfg.swarm.offsetTransitions,tk);
    end
    controlCfg=cfg;
    ...
    Vlog(k,:,:) = V;
    Alog(k,:,:) = accCmd;
    DesiredOffsetsLog(k,:,:) = currentOffsets;
    ...
    if isfield(cfg.mac,'interferenceTransitions')
        net.mac.interferenceMatrix=interferenceMatrixAtTime( ...
            cfg.mac.interferenceMatrix,cfg.mac.interferenceTransitions,tk);
    end
...
out.contextAwareConfig = cfg.shared.contextAware;
out.serviceScheduler = net.serviceScheduler;
...
valid = {'periodic','periodic-feedback','state-event', ...
    'causal-aoi-only','causal-aoci', ...
    'delayed-ack-belief','causal-broadcast','mac-aware-broadcast', ...
    'context-aware-broadcast','load-guarded-broadcast','causal-unicast'};
if ~any(strcmp(method,valid))
    error(['simSwarmSharedMedium: unknown method. Expected periodic, ' ...
        'periodic-feedback, state-event, causal-aoi-only, causal-aoci, ' ...
        'delayed-ack-belief, ' ...
        'causal-broadcast, mac-aware-broadcast, context-aware-broadcast, ' ...
        'load-guarded-broadcast, or causal-unicast.']);
end
...
elseif strcmp(method,'context-aware-broadcast')
    requested = 'adaptive';
    ...
    if ~strcmp(requested,'adaptive')
        error(['simSwarmSharedMedium: context-aware-broadcast requires ' ...
            'feedbackMode=adaptive.']);
    end
    if ~cfg.shared.contextAware.enabled
        error(['simSwarmSharedMedium: context-aware-broadcast requires an ' ...
            'enabled contextAware policy.']);
    end
