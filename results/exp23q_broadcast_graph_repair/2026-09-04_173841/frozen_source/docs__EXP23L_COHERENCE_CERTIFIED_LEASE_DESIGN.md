...
(r) is an intended DATA recipient of sender (i). Because an intended DATA
link is detectable by definition, define the effective detectability relation
(D(t)=I(t)\lor A). The actual broadcast sender-conflict graph (F(t)) contains
({i,j}) whenever (i) and (j) are half-duplex neighbors or either sender is
detectable at an intended receiver of the other:

\[
 F_{ij}(t)= A(j,i)\lor A(i,j)
 \lor \bigvee_{r:A(r,i)=1} D_{rj}(t)
 \lor \bigvee_{r:A(r,j)=1} D_{ri}(t).
\]
...
using (D^+(s,H)=I^+(s,H)\lor A) in the receiver-lift equation. Both
...
2. (I^+(s,H)) contains every physical interference relation permitted by the
   reachable tubes, and (F^+(s,H)) is the receiver lift of
   (D^+(s,H)=I^+(s,H)\lor A);
