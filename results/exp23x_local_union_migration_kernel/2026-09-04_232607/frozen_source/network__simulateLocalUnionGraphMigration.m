...
migrateSeen=false(E,1); neighborSeen=false(E,1); receipt=false(E,1);
...
for frame=1:F
    ...
    if transactionActive
        ...
        for e=1:E
            w=witnesses(e);
            if w==node || A.success(w), migrateSeen(e)=true; end
        end
        ...
        ready=migrateSeen & neighborSeen & ~receipt;
...
retryFrames=max(0,F-C.eligibleFrame+1);
uniqueWitnesses=numel(unique(witnesses));
attemptBound=1+retryFrames*(1+numel(unique(neighbors))+uniqueWitnesses);
...
byteBound=M.config.revokeBytes+retryFrames*( ...
    (1+numel(unique(neighbors)))*M.config.claimBytes+ ...
    responseBytesPerRetry);
...
requiredConfig={'maxFrames','transitionFrame','eligibleFrame', ...
    'newGraphActivationFrame','phyRateBps'};
