...
for c = 1:nCell
    ...
    for s = 1:numSeeds
        ...
        %     no reverse channel" ---
        if ACKHASH(s,IDX_CAUSAL,c) ~= reg.ack(s,col)
            hashProblems{end+1} = sprintf( ...
                '%s seed %d: Causal reverse hash is not the registry value', ...
                cellLabel{c}, exp10Seeds(s));                       %#ok<AGROW>
        end

        if any(~isnan(ACKHASH(s,[IDX_P10 IDX_P20 IDX_EVENT],c)))
            hashProblems{end+1} = sprintf( ...
        ...
        hashProblems = localCheckPerturbHash(hashProblems, ...
            'external force', EXTHASH(s,:,c), reg.extForce(s,col), ...
            strcmp(pt.kind,'mismatch'), cellLabel{c}, exp10Seeds(s));

        hashProblems = localCheckPerturbHash(hashProblems, ...
            'estimator noise', NOISEHASH(s,:,c), reg.noise(s,col), ...
