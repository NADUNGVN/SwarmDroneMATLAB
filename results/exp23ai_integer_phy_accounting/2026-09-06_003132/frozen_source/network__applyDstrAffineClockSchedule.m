...
Dc=D;
if isfield(nativeQ,'controlAirtimeSec')
    Dc=nativeQ.controlAirtimeSec;
end
nativeControlDuration=repmat(Dc,size(nativeQ.controlStartTime));
if isfield(nativeQ,'controlAttemptAirtimeSec')
    nativeControlDuration=nativeQ.controlAttemptAirtimeSec;
end
if numel(nativeControlDuration)~=numel(nativeQ.controlStartTime) || ...
        any(~isfinite(nativeControlDuration) | nativeControlDuration<=0)
    error('applyDstrAffineClockSchedule: invalid control durations.');
end
...
Q.dataGroup=group;
if isfield(nativeQ,'dataKind')
    Q.dataKind=nativeQ.dataKind(keep);
end
...
if ~isempty(Q.dataStartTime)
    ...
    if isfield(Q,'dataKind'), Q.dataKind=Q.dataKind(order,:); end
end
...
controlDuration=zeros(0,1);
controlBytes=zeros(0,1);
q=0;
...
for outIndex=1:numel(keptIndices)
    ...
    controlDuration(q+1:q+m,1)=durations;
    if isfield(nativeQ,'controlAttemptBytes')
        controlBytes(q+1:q+m,1)=nativeQ.controlAttemptBytes(member);
    end
    if isfield(nativeQ,'controlKind')
        controlKind(q+1:q+m,1)=nativeQ.controlKind(member);
    end
    q=q+m;
end
if ~isempty(controlActual)
    ...
    controlDuration=controlDuration(order);
    if isfield(nativeQ,'controlAttemptBytes')
        controlBytes=controlBytes(order);
    end
    if isfield(nativeQ,'controlKind'), controlKind=controlKind(order); end
end
...
Q.controlAttemptAirtimeSec=controlDuration;
if isfield(nativeQ,'controlAttemptBytes')
    Q.controlAttemptBytes=controlBytes;
end
if isfield(nativeQ,'controlKind'), Q.controlKind=controlKind; end
...
if isfield(nativeQ,'controlGroupBytes')
    Q.controlGroupBytes=nativeQ.controlGroupBytes(keepControl,:);
end
if isfield(nativeQ,'controlPhyRateBps')
    Q.controlPhyRateBps=nativeQ.controlPhyRateBps;
end
if isfield(nativeQ,'controlGroupAirtimeSec')
    Q.controlGroupAirtimeSec= ...
        nativeQ.controlGroupAirtimeSec(keepControl,:);
else
    Q.controlGroupAirtimeSec=zeros(nnz(keepControl),1);
    for g=1:nnz(keepControl)
        member=Q.controlFrame==Q.controlGroupFrame(g) & ...
            Q.controlSlot==Q.controlGroupSlot(g);
        Q.controlGroupAirtimeSec(g)=sum(Q.controlAttemptAirtimeSec(member));
    end
end
...
Q.expectedManagementAttempts=sum(Q.controlGroupAttempts);
if isfield(Q,'controlGroupBytes')
    Q.expectedManagementBytes=sum(Q.controlGroupBytes);
end
if isfield(Q,'expectedManagementBytes')&&isfield(Q,'controlPhyRateBps')
    Q.expectedManagementAirtime= ...
        8*Q.expectedManagementBytes/Q.controlPhyRateBps;
else
    Q.expectedManagementAirtime=sum(Q.controlGroupAirtimeSec);
end
Q.expectedManagementRecipientAttempts=sum( ...
    Q.controlGroupRecipientAttempts);
Q.expectedManagementRecipientSuccess=sum( ...
    Q.controlGroupRecipientSuccess);
Q.expectedManagementRecipientErasure=sum( ...
    Q.controlGroupRecipientErasure);
Q.expectedManagementRecipientCollision=sum( ...
    Q.controlGroupRecipientCollision);
...
Q.hashExact=realizationHash([nativeQ.hashExact;H;D;nativeQ.guardSec; ...
    ...
    Q.controlFrame;Q.controlAttemptAirtimeSec; ...
    Q.controlGroupNominalStartTime; ...
    Q.controlGroupStartTime;Q.controlGroupEndTime; ...
    Q.controlGroupAttempts;Q.controlGroupAirtimeSec; ...
    Q.controlGroupRecipientAttempts; ...
...
for g=1:numel(Q.controlGroupStartTime)
    ...
    member=Q.controlFrame==Q.controlGroupFrame(g) & ...
        Q.controlSlot==Q.controlGroupSlot(g);
    starts=Q.controlStartTime(member);
    if ~isempty(starts)
        maxSkew=max(maxSkew,max(starts)-min(starts));
    end
end
