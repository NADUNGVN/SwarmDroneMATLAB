...
if row.REPLAY_FLAG
    ...
    row.REPLAY_MANAGEMENT_ATTEMPT_MATCH=double( ...
        S.dstrManagementAttempts==Q.expectedManagementAttempts);
    row.REPLAY_MANAGEMENT_RECIPIENT_MATCH=double( ...
        S.dstrManagementRecipientAttempts== ...
    ...
    row.REPLAY_MANAGEMENT_AIRTIME_MATCH=double(abs( ...
        S.dstrManagementAirtime-Q.expectedManagementAttempts* ...
        controlAirtime)<=1e-9);
    ...
    if strcmp(family,'candidate-elcs')
        ...
        row.ELCS_GRANT_WITHOUT_DECODED_REQUEST= ...
            K.grantWithoutDecodedRequest;
        row.ELCS_CONTROL_ATTEMPT_BOUND=Q.controlAttemptBound;
        row.ELCS_CONTROL_BOUND_RATIO= ...
            (K.statusAttempts+K.grantAttempts)/Q.controlAttemptBound;
