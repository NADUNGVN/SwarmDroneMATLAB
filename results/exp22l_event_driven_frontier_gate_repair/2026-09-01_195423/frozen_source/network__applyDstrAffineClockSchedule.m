...
[timingSafe,minGap,maxSkew]=timingCertificate(Q,D,Dc);
...
function [safe,minGap,maxSkew]=timingCertificate(Q,D,Dc)
...
for g=1:numel(Q.controlGroupStartTime)
    ...
    duration=finish(end)-start(end);
    maxSkew=max(maxSkew,duration-Dc);
end
if isempty(start)
