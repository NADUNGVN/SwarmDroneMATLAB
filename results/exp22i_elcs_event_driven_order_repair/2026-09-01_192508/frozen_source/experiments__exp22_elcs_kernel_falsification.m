...
switch lower(char(variant))
    ...
    case 'v5'
        ...
        experimentName='exp22h_elcs_event_driven_renewal';
    case 'v6'
        R=exp22iElcsKernelRegistry();
        experimentName='exp22i_elcs_event_driven_order_repair';
    otherwise
...
if isfield(R,'grantRequiresDecodedRequest') && ...
    ...
    requestDiscipline=all(T.GRANT_WITHOUT_DECODED_REQUEST==0) && ...
        all(T.STATUS_ATTEMPTS==T.DISCOVERY_STATUS_ATTEMPTS+ ...
            T.REQUEST_STATUS_ATTEMPTS);
    add('event_driven_request_discipline',requestDiscipline,sprintf( ...
        '%d GRANTs without request; STATUS partition residual %d', ...
        sum(T.GRANT_WITHOUT_DECODED_REQUEST), ...
        sum(T.STATUS_ATTEMPTS-T.DISCOVERY_STATUS_ATTEMPTS- ...
            T.REQUEST_STATUS_ATTEMPTS)));
    zeroControl=all(T.CONTROL_ATTEMPTS(zero)<= ...
        T.CONTROL_ATTEMPT_BOUND(zero)) && ...
        all(T.CONTROL_BOUND_RATIO(zero)<=1+1e-12);
    add('analytical_zero_loss_control_bound',zeroControl,sprintf( ...
        'maximum observed/bound ratio %.6f', ...
        max(T.CONTROL_BOUND_RATIO(zero))));
    deadline=all(T.FIRST_ALL_CERTIFIED_FRAME(zero)<=T.N(zero));
    add('zero_loss_acquisition_deadline',deadline,sprintf( ...
        'maximum first-certification excess %.0f frames', ...
        max(T.FIRST_ALL_CERTIFIED_FRAME(zero)-T.N(zero))));
end
...
root=projectRoot();
if startsWith(R.version,'EXP22I')
    plan='docs/EXP22I_EVENT_DRIVEN_ORDER_REPAIR_PLAN.md';
    registry='utils/exp22iElcsKernelRegistry.m';
elseif startsWith(R.version,'EXP22H')
    plan='docs/EXP22H_EVENT_DRIVEN_RENEWAL_PLAN.md';
    registry='utils/exp22hElcsKernelRegistry.m';
...
files={'docs/EXP22_EDGE_LEASE_CANDIDATE_DESIGN.md',plan, ...
    ...
    'utils/exp22hElcsKernelRegistry.m', ...
    'utils/exp22iElcsKernelRegistry.m', ...
    'utils/elcsControlAttemptBound.m', ...
