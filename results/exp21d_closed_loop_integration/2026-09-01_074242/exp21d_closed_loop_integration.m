...
add('native_kernel_terminal_valid',all(T.DSTR_KERNEL_FINAL_RESOLVED(native)==1) && ...
    all(T.DSTR_KERNEL_FINAL_COLLISION_FREE(native)==1) && ...
    all(T.DSTR_KERNEL_FINAL_FRAME_AGREEMENT(native)==1), ...
    'every native kernel terminal schedule is resolved and valid');
