...
if strcmp(net.serviceScheduler.config.mode,'continuous-dstr-replay')
...
while cursor<t1-tol
    ...
    if nextControl<=cursor+tol
        ...
        S.dstrControlActiveEnd=max(S.dstrControlActiveEnd, ...
            cursor+Q.airtimeSec);
    ...
    if nextData<=cursor+tol
        ...
        S.continuousStartOpportunities=S.continuousStartOpportunities+ ...
            opportunities;
        net.serviceScheduler=S;
        net=applyDstrHalfDuplexMasks(net,candidates);
...
function net=applyDstrHalfDuplexMasks(net,candidates)

txMask=reshape(logical(candidates),1,[]);
