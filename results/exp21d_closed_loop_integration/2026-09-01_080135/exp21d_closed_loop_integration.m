...
dstrTrace=generateExp21dDstrTrace(seedValue,C.N,R);
nativeQ=buildDstrContinuousSchedule(C,dstrTrace,base.swarm.T);
rows=repmat(exp21dClosedLoopEmptyRow(),numel(R.arms),1);
for k=1:numel(R.arms)
    arm=R.arms(k);
    [cfg,method,label,details]=applyExp21dClosedLoopArm(base,arm,nativeQ);
