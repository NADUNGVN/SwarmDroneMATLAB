...
| objected slots enter `failed_shrink`; `FST` expires entries and removal shifts cache indices | Sec. 4.4.1, pp. 15--16 | required with source-evaluated `FST=10` |
| initial shrink backoff follows the node's order in the superframe; NACK-only retries use truncated exponential backoff | Sec. 4.4 and 4.4.2, pp. 15--16 | own local transmission-slot index is the causal order proxy; duplicate reuse slots are resolved by the published exponential retry |
