function exp18a4_capacity_gated_selector_development()
%EXP18A4_CAPACITY_GATED_SELECTOR_DEVELOPMENT Exact-route development audit.

startup;
close all;
runScriptIsolated('test_context_aware_policy');
runScriptIsolated('test_exp18a4_contracts');
runScriptIsolated('test_exp18a4_analysis_contracts');
R=exp18a4Registry();
referencePath=fullfile(R.referenceRun,'tidy.csv');
if exist(referencePath,'file')~=2
    error('exp18a4: canonical EXP18A reference is missing.');
end
[registryHash,registryLeaves]=configHash(R);
expRun=startExperiment('exp18a4_capacity_gated_selector_development', ...
    ['Development-only capacity abstention with exact fixed-route alias; ' ...
    'no confirmatory claim or new threshold.']);
...
rows=repmat(exp18a4EmptyRow(),0,1);
batchSize=16;
...
if all(gates.passed==1)
    analysis=analyzeExp18A4Development(expRun.dir);
    fprintf('\nEXP18A4 DEVELOPMENT VERDICT: %s (%d/%d gates)\n', ...
...
if ~all(gates.passed==1)
    error('exp18a4_capacity_gated_selector_development: integrity failure.');
end
...
[cfg,method,label,details,certificate]=applyExp18A4Arm(base);
meta=struct('stage','development-amendment-4','scenario',c.id, ...
    'scenarioLabel',c.label,'family','EXP18A4 capacity-gated selector', ...
...
row=runExp18A4Cell(cfg,method,label,meta,trace,details,certificate);
...
[names,passed,detail]=addGate(names,passed,detail,'matrix_completeness', ...
    height(T)==R.expectedRuns,sprintf('%d selector runs',height(T)));
...
[names,passed,detail]=addGate(names,passed,detail,'exact_reference_trace', ...
    traceMatch,'selector matches canonical EXP18A realizations');
...
semantics=all(string(T.method)=="context-aware-broadcast") && ...
    ...
    all(string(T.route)=="capacity-gated-selector") && ...
    all(string(T.routeRule)=="capacity-gated-mac") && ...
    all(T.serviceCertificateEnabled==1);
[names,passed,detail]=addGate(names,passed,detail,'candidate_semantics', ...
    semantics,'capacity certificate and closed route map are enabled');
...
root=projectRoot();
files={'docs/EXP18A4_CAPACITY_GATED_SELECTOR_PLAN.md', ...
    'utils/exp18a4Registry.m','utils/applyExp18A4Arm.m', ...
    'utils/exp18a4EmptyRow.m','utils/runExp18A4Cell.m', ...
    ...
    'tests/test_context_aware_policy.m','tests/test_exp18a4_contracts.m', ...
    'tests/test_exp18a4_analysis_contracts.m', ...
    'experiments/analyzeExp18A4Development.m', ...
    'experiments/exp18a4_capacity_gated_selector_development.m'};
...
fid=fopen(path,'w');
if fid<0, error('exp18a4: cannot write %s.',path); end
