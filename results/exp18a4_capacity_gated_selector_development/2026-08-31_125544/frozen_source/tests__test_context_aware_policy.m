...
gatedCsma=csma;
gatedCsma.shared.contextAware.routeRule='capacity-gated-mac';
gatedCsma.shared.contextAware=contextAwarePolicyConfig(gatedCsma);
netCsma=initSharedMediumState(gatedCsma,A);
netCsma=enqueueAckSummary(netCsma,2,entry,0,gatedCsma);
[allowGatedCsma,~,infoGatedCsma]= ...
    adaptiveStandaloneAckDecision(netCsma,2,0.10,gatedCsma);
gatedAloha=aloha;
gatedAloha.shared.contextAware.routeRule='capacity-gated-mac';
gatedAloha.shared.contextAware=contextAwarePolicyConfig(gatedAloha);
[allowGatedAloha,~,infoGatedAloha]= ...
    adaptiveStandaloneAckDecision(net,2,0.02,gatedAloha);
checks(end+1,:)={~allowGatedCsma && allowGatedAloha && ...
    strcmp(infoGatedCsma.reason,'feasible-piggyback-route') && ...
    strcmp(infoGatedAloha.reason,'feasible-aloha-adaptive'), ...
    'capacity-gated route maps feasible CSMA to piggyback and ALOHA to adaptive'};
