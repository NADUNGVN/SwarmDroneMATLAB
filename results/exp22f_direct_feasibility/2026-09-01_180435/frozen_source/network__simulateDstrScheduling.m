...
debugShrinkCandidate=zeros(F,N);
debugShrinkBackoff=zeros(F,N);
debugFailedShrinkCount=zeros(F,N);
debugAssigned=zeros(F,N);
debugState=zeros(F,N,'uint8');
debugTxTG=false(F,N);
debugTxTGn=false(F,N);
debugTxTS=false(F,N);
debugTxTSo=false(F,N);
debugTxTSn=false(F,N);
debugTxSlotThis=zeros(F,N);
debugDataSuccess=false(F,N,N);
debugDataErasure=false(F,N,N);
debugDataCollision=false(F,N,N);
debugTsPayload=zeros(F,N);
debugApplyShrink=false(F,N);
...
for frame=1:F
    ...
    for proposer=reshape(txTS,1,[])
        ...
        elseif ~successApplied
            ...
            width=2^min(shrinkFailures(proposer), ...
                C.shrinkBackoffExponentCap);
    ...
    for dataSlot=1:maxSlotsThis
        ...
        debugDataSuccess(frame,:,:)=reshape( ...
            squeeze(debugDataSuccess(frame,:,:)) | aData.successMask,N,N);
        debugDataErasure(frame,:,:)=reshape( ...
            squeeze(debugDataErasure(frame,:,:)) | aData.erasureMask,N,N);
        debugDataCollision(frame,:,:)=reshape( ...
            squeeze(debugDataCollision(frame,:,:)) | ...
            aData.collisionMask,N,N);
    ...
    % from already discovered reporters.  Section 4.5 permits temporary
    % acceptance before every neighbor has ever been discovered, but every
    % already-known reporter must return fresh success for a positive claim.
    % Any received negative dominates.  An Assignment attempt with missing
    % known reports times out causally after this complete superframe and
    % retries; no receiver result is fabricated.
    ...
    if C.enableShrink
        ...
        for node=reshape(newEligible,1,[])
            ...
            % The configured formation order supplies the source-defined
            % unique initial backoff.  It is prior metadata, not runtime
            % schedule truth; spatial-reuse peers therefore do not alias.
            shrinkBackoff(node)=C.shrinkInitialOrder(node)-1;
        end
    end
    ...
    debugShrinkCandidate(frame,:)=shrinkCandidate;
    debugShrinkBackoff(frame,:)=shrinkBackoff;
    debugFailedShrinkCount(frame,:)=sum(failedShrinkAge>0,2);
    debugAssigned(frame,:)=assigned;
    debugState(frame,:)=state;
    debugTxTG(frame,txTG)=true;
    debugTxTGn(frame,txTGn)=true;
    debugTxTS(frame,txTS)=true;
    debugTxTSo(frame,txTSo)=true;
    debugTxTSn(frame,txTSn)=true;
    debugTxSlotThis(frame,:)=txSlotThis;
    debugTsPayload(frame,:)=tsPayload;
    debugApplyShrink(frame,:)=applyShrink;
end
...
out.debug=struct('shrinkCandidate',debugShrinkCandidate, ...
    'shrinkBackoff',debugShrinkBackoff, ...
    'failedShrinkCount',debugFailedShrinkCount, ...
    'assigned',debugAssigned,'state',debugState, ...
    'txTG',debugTxTG,'txTGn',debugTxTGn,'txTS',debugTxTS, ...
    'txTSo',debugTxTSo,'txTSn',debugTxTSn, ...
    'txSlotThis',debugTxSlotThis, ...
    'dataSuccess',debugDataSuccess, ...
    'dataErasure',debugDataErasure, ...
    'dataCollision',debugDataCollision, ...
    'tsPayload',debugTsPayload,'applyShrink',debugApplyShrink);
...
required={'N','maxFrames','initialDataSlots','maxDataSlots', ...
    ...
    'failedShrinkTimeout','shrinkBackoffExponentCap', ...
    'shrinkInitialOrder','enableShrink','churnEnabled','churnFrame', ...
    'churnNode'};
...
integerPositive(C.shrinkBackoffExponentCap, ...
    'shrinkBackoffExponentCap');
if ~isequal(sort(C.shrinkInitialOrder(:)),(1:C.N)')
    error(['simulateDstrScheduling: shrinkInitialOrder must be a ' ...
        'permutation of 1:N.']);
end
C.shrinkInitialOrder=C.shrinkInitialOrder(:);
...
x=[C.N C.maxFrames C.initialDataSlots C.maxDataSlots ...
    ...
    C.failedShrinkTimeout C.shrinkBackoffExponentCap C.enableShrink ...
    C.churnEnabled C.churnFrame C.churnNode ...
    C.shrinkInitialOrder' ...
