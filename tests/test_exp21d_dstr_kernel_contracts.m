...
checks(end+1,:)={R.expectedRuns==800 && numel(R.seeds)==100 && ...
    ...
    R.initialDataSlots==10 && R.failedShrinkTimeout==30 && ...
    R.maxFrames==1000 && R.nativeResolutionDeadline==200, ...
...
checks(end+1,:)={isequal(Cm.neighborGraph,Cn.neighborGraph) && ...
    ...
    isequal(Cn.shrinkInitialOrder,(1:10)') && ...
...
%% V2's retained internal-gap witness closes under the source FST=30 arm.
Rgap=R;
Rgap.maxFrames=250;
Tgap=generateExp21dDstrTrace(16034002,10,Rgap);
[Cgap,~]=applyExp21dDstrCondition(10,'native',Rgap);
gap=simulateDstrScheduling(Cgap,Tgap);
used=unique(gap.assignedSlot(gap.assignedSlot>0))';
checks(end+1,:)={gap.finalAllResolved && gap.finalCollisionFree && ...
    gap.frameLog(end).converged && ...
    isequal(used,1:gap.finalDataSlots), ...
    'source FST=30 removes a retained v2 internal-gap witness'};
