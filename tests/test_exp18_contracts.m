...
rowBase=base;
rowBase.mac.type='csma';
[rowCfg,rowMethod,rowLabel,rowDetails]=applyExp18Arm( ...
    rowBase,R.arms(strcmp({R.arms.id},'frame-piggyback')));
meta=struct('stage','development','scenario','n5-moderate-bg0', ...
    'scenarioLabel','N5 Moderate bg0','family','EXP18 test', ...
    'arm','frame-piggyback','pointIndex',2,'parameterValue',0, ...
    'role','core','channel','moderate','modifier','nominal', ...
    'basePAccess',1/5);
row=runExp18Cell(rowCfg,rowMethod,rowLabel,meta,trace,rowDetails);
checks(end+1,:)={isequal(fieldnames(row),fieldnames(exp18EmptyRow())), ...
    'every disabled/enabled context arm returns the exact EXP18 row schema'};
