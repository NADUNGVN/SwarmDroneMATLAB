...
arm = struct('id',"PVTEST",'family',"Predictive-VoI", ...
    'label',"Predictive-VoI",'kind',"predictive-voi", ...
    'parameterName',"voiPrice",'parameterValue',1e-6, ...
    'period_s',NaN,'epsilon_m',NaN,'retry_s',NaN, ...
    'voiPrice',1e-6,'voiHorizonSamples',25);
shortScenario = struct('id','S1','name','short-static', ...
    'evaluationStart_s',0,'eventWindows_s',[1 3], ...
    'performanceWindows_s',[0 4]);
frontierRow = runTcnsFrontierCell(cfg,arm,shortScenario);
assert(~frontierRow.failed && isfinite(frontierRow.voiMeanScore) && ...
    isfinite(frontierRow.voiEventAllocationRatio) && ...
    isequal(fieldnames(frontierRow),fieldnames(tcnsFrontierRow())), ...
    'PredictiveVoI: stable frontier-runner contract failed.');
