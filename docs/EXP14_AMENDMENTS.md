...
## Amendment 005 — complete the preregistered AoI summaries

After EXP14B had opened but before any EXP14A or EXP14B performance value was
read, a static audit of `experiments/analyzeExp14Results.m` found that the raw
row contract logged mean, p95 and p99 true AoI as preregistered, while the
post-run summary and five named paired contrasts exposed only mean true AoI.

The analysis script now adds p95 and p99 true AoI as additive summary columns
and applies the already frozen paired-t and deterministic paired-bootstrap
procedure to both metrics. No raw simulation output, seed, policy, baseline,
OOD point, exclusion rule, dominance definition, interval method or integrity
gate changes. The original analysis source remains in both run snapshots and
the post-amendment source hash is recorded before performance inspection.

The same pre-performance audit also made the OOD interpretation rule explicit.
For every one of the eight fixed OOD points, the secondary analysis reports the
same five comparisons: hybrid minus P10, P20, state-event and delayed-belief,
plus piggyback minus hybrid. It applies the same five continuous outcomes and
the same paired intervals, and reports the two raw safety proportions with
Wilson intervals. These 40 OOD comparisons are exhaustive and labelled
secondary/descriptive with unadjusted intervals; they are not new confirmatory
hypotheses or machine pass gates.
