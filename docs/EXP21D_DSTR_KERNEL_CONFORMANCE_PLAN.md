...
**Status:** v3 frozen after retained invalid v1/v2 runs; see Amendments 01--03
...
- failed-shrink cache timeout `FST = 30` superframes;
- time-slot retention probability `TSR = 0.75`;
- 512-byte beacon/control frame;
- 1 Mbit/s reference PHY for airtime accounting; and
- native allocation-resolution deadline 200 superframes; and
- maximum 1000 superframes for conservative unused-slot removal and all
  retained boundary trajectories.
...
One hundred fresh v3 seeds `16035001:16035100` are used for each of
...
5. every native row also converges with no globally unused transmission slot
   within the separately registered 1000-superframe cap;
...
The executable registry requires 16 deterministic contract checks.  The run
stores a thirteen-file frozen-source manifest and generates aggregate summaries
