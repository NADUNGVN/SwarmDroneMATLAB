...
Distributed multi-agent event-triggered communication has also been learned
jointly with control. Kesper et al. append explicit age timers to the last
broadcast states available to each agent in a decentralized-execution
multi-agent reinforcement-learning policy~\cite{kesper2023toward}. Their
timer resets when an agent broadcasts; it is not a delayed cumulative-ACK
confirmation of what a particular receiver holds, and the method has no
new-information/refresh semantic split.
...
network~\cite{wang2021freshness}. Lin et al. place AoI in both an
event-triggered communication scheduler and an LQR load-frequency
controller~\cite{lin2023eventtriggered}. These works establish direct
AoI-aware event-triggered control precedent, but do not infer a particular
receiver's confirmed state from delayed cumulative acknowledgements or
separate new information from refresh traffic. Thus ``AoI-aware event
triggering'' is not itself the gap addressed here.
...
Bidirectional AoI-based event triggering also predates this work. Onozuka
et al. apply an AoI-based event trigger to both forward and feedback
communication in networked railway vehicle control~\cite{onozuka2024aoi}.
That is important precedent for using freshness in both directions of a
control loop, but it is not a distributed multi-agent/UAV formation method
and does not implement cumulative-ACK-confirmed innovation or our
new-information/refresh in-flight mechanism.
...
\item event-triggered multi-agent and UAV-formation designs establish local
      ...
      freshness-aware triggering, and Kesper et al. additionally combine
      last-broadcast states with age timers in learned distributed
      multi-agent control; none supplies the full mechanism studied here
      (\S\ref{sec:rel_etc});
...
\item ACK-driven causal receiver-AoI inference already exists in both
      ...
      while bidirectional AoI/event triggering appears in railway control;
      these lack the physical state-innovation and new-versus-refresh
      in-flight semantic split (\S\ref{sec:rel_ack});
