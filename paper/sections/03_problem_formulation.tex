...
cascaded attitude controller at a $10{:}1$ inner-to-outer step ratio,
following the standard geometric and differentially flat quadrotor
formulations~\cite{lee2010geometric,mellinger2011minimum}. The
...
\item \textbf{delay} $d + \sigma_j z$, where $\sigma_j$ is the jitter
      standard deviation, $z$ is a standard normal draw, and the result is
      floored at zero,
...
as an ideal-feedback reference --- but it is not implementable, and its efficiency
...
Equation~\eqref{eq:costbcast} is an accounting proxy that collapses the
unicast links leaving one sender in one tick into one idealized broadcast
transmission; acknowledgements remain unicast. It is not a simulation of
medium access, contention, or broadcast-channel delivery.
