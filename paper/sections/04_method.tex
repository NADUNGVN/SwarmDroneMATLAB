...
ACK-confirmed generation time $g^{\mathrm{ack}}$ and sequence
$k^{\mathrm{ack}}$, the last transmission time $t^{\mathrm{tx}}$, and the
outstanding sequence set $\mathcal O$. An arrived cumulative ACK
$\langle k^a,g^a\rangle$ reports its accepted sequence and generation time.
...
$t-t^{\mathrm{tx}}\geq\tau_{\min}$, with hard position/velocity thresholds
$\epsilon_p,\epsilon_v$, refresh cooldown $\tau_r$, and max-silence interval
$\tau_{\max}$, the sender evaluates four branches in order:
\begin{align}
...
The policy is distributed in the operational sense that each sender uses only
its own state and ACKs received on its incident channels; it is not
feedback-free and requires no global coordinator or regime label.
