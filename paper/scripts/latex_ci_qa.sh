...
failures=0
for value in "$undefined_citations" "$undefined_references" "$duplicate_labels" "$missing_artifacts" "$float_problems"; do
  if [[ "$value" -ne 0 ]]; then
    failures=1
  fi
done
...
if [[ "$reference_count" -ne 53 || "$figure_count" -ne 13 || "$table_count" -ne 8 ]]; then
...
echo "LATEX_CI_QA_OK citations=0 refs=0 duplicate-labels=0 missing-artifacts=0"
