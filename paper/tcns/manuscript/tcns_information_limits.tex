...
performance output.  The conditional fixed-response quadratic action benefit
is an affine
...
minimum number of instantaneous noiseless linear scalar statistics needed for
several action values equals the rank of their unobservable coefficient
projection.
...
controller-relevant structural unit-response value functionals violate
sender-history identifiability in the evaluated information topology.  Two
full, unsaturated, dynamically
...
charging an ACK-like receiver-information stream can remove that headroom
under the frozen packet-frequency metric.  The results delimit exact local
value-based communication; they do
...
sender-history nonidentifiability for ten structural unit-response payload
functionals, construct two reachable
opposite-sign histories with positive unavoidable regret, and connect the
value of receiver information to a frozen ACK-like break-even diagnostic.
\end{enumerate}
...
Pareto headroom in S2 but not in the S4 congestion boundary.  O1 is privileged,
nondeployable, and the comparison is non-inferential.}
\label{fig:oracle}
\end{figure*}
...
performance-matchable O1 development points (median and IQR).  The dashed
...
For each controller-relevant payload we evaluate the structural nonzero
unit command correction \([1,0,0]^\top\), with zero substituted receiver
payload, which fixes \(g\) on the compatible information fiber.  Across all
ten such structural unit-response functionals, the maximum discrepancy between
the affine representation and the centralized O1 computation is
\(4.34\times10^{-16}\).
...
horizon.  Thus Table~\ref{tab:links} uses these reduced finite-history maps.
The later sender-wise simultaneous-action audit instead uses each sender's
current observation map on the common full 99-dimensional state.

\begin{table*}[t]
\caption{N=5 finite-history structural unit-response functional and rank
audit.  Ordinary/pinned
...
Table~\ref{tab:links} shows that all ten structural unit-response value
coefficients lie outside the
...
This audit does not assert nonidentifiability for every possible physical
correction: a zero response has zero value, and special nonzero response
combinations can cancel a hidden projection.  The two trajectory witnesses
below use actual nonzero response corrections rather than the structural unit
surrogate.
...
increases rank by one for all ten unit-response functionals.  At
\(10^{-14}\), three follower
...
Both witnesses therefore exercise sender 1; they distinguish ordinary from
pinned-leader receiver-memory semantics, not sender diversity or prevalence
across the graph.
...
belief-weighted values are \(\mp4.95740\times10^{-6}\).  All values and
regrets in this subsection are in \(\mathrm{m}^2\) under the sampled
per-follower normalization following \eqref{eq:formation-value}.  The maximum sender
...
\begin{table*}[t]
\caption{Reachable local value uncertainty and one-decision regret
(\(\mathrm{m}^2\), threshold \(\tau=0\)).}
\label{tab:regret}
\centering
\scriptsize
\begin{tabular}{lrrrr}
...
leader-sender witnesses, covering ordinary and pinned-leader memories.  The
sender's complete allowed observation history and each actual candidate
response are identical, although hidden follower evolution and the conditional
fixed-response value sign differ.}
\label{fig:witness}
\end{figure*}
...
O1 development points within the measured periodic performance domain and do
...
In the evaluated five-UAV formation topology, all ten structural unit-response
value functionals violate sender-history identifiability, and two reachable
history pairs require opposite exact decisions while remaining
indistinguishable to the leader sender.
The corresponding local randomized minimax regrets are
\(2.65\times10^{-6}\) and \(2.48\times10^{-6}\); deterministic regrets are
twice as large.  Their scale certifies strict decision divergence in the
constructed information fibers; it is not asserted to be a mission-level
engineering loss.  Four leader actions share three missing directions, while
