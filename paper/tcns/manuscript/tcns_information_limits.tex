...
finite-history agent information by row-space conditions.  We then distinguish
exact-value recovery from value-sign decisions, derive a local sign-ambiguity
certificate, and prove that the minimum number of supplementary linear scalar
statistics needed for several action values equals the rank of their
unobservable coefficient projection.  Coalition ownership of those statistics
and their communication acquisition cost are treated separately.
...
all five agents to synthesize it in this topology.  Frozen development
...
\item We specialize functional observability to this action value, cover
finite local histories, and establish a value-sign ambiguity test on the
information-compatible operating set.
\item We prove that the minimum dimension of extra linear information for a
family of action values is the rank of its projection outside the available
information row space.  We separately characterize coalition ownership,
because statistic dimension is not distributed acquisition cost.
\item For the sampled N=5 formation architecture, we validate the affine map
to machine precision, show sender-history nonidentifiability for all ten
payloads, construct two reachable opposite-sign histories, and connect the
...
characterizations and target-sensor questions are established, including for
large networks \cite{montanari2022functional}.  Our theoretical core uses
that foundation with the exact quadratic communication-action value as the
functional, then adds a decision-sign result and an acquisition-economics
interpretation specific to control communication.
...
& Row-space tests and target-state/sensor selection
& What do these tests imply for action sign and acquisition economics?
& Uses established theory, adds action-specific sign, statistic/acquisition separation, and formation closure. \\
\bottomrule
\end{tabular}
\end{table*}
...
\subsection{Value-sign decidability}

Exact magnitude is stronger than the sign needed by a transmit-if-beneficial
rule.  Let \(\mathcal X\) be the declared operating set and
\begin{equation}
...
\begin{theorem}[Sign ambiguity]
\label{thm:sign}
If \(q_{\min}(o)<0<q_{\max}(o)\), no deterministic policy based only on o
can recover the sign of the exact action value for every state in
\(\mathcal X(o)\).
\end{theorem}

\begin{proof}
The same o is compatible with at least one negative-value state and one
positive-value state.  A deterministic map of o returns the same decision in
both cases and is therefore wrong in at least one.
\end{proof}
...
Theorem~\ref{thm:sign} is local to \(\mathcal X(o)\).  It does not rule out a
...
\begin{table*}[t]
\caption{N=5 finite-history action-value identifiability.  The residual is
\(\delta_{\mathcal O_L}((A_0^L)^\top\ell)\) from
\eqref{eq:residual}.  Every strictly positive entry implies
nonidentifiability under the stated tolerance.}
\label{tab:links}
\centering
\scriptsize
\begin{tabular}{llrrrrl}
\toprule
Payload & Link & Reduced n & L & History rank & Normalized residual & Exact value identifiable? \\
\midrule
ordinary & $1\to2$ & 93 & 92 & 9  & 0.0560 & no \\
ordinary & $1\to5$ & 93 & 92 & 9  & 0.0102 & no \\
ordinary & $2\to3$ & 93 & 92 & 27 & 0.9999 & no \\
ordinary & $3\to2$ & 93 & 92 & 18 & 0.9999 & no \\
ordinary & $3\to4$ & 93 & 92 & 18 & 0.9999 & no \\
ordinary & $4\to3$ & 93 & 92 & 27 & 0.9999 & no \\
ordinary & $4\to5$ & 93 & 92 & 27 & 0.9999 & no \\
ordinary & $5\to4$ & 93 & 92 & 18 & 0.9999 & no \\
pinned leader & $1\to2$ & 90 & 89 & 9 & 0.0041 & no \\
pinned leader & $1\to4$ & 90 & 89 & 9 & 0.0036 & no \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table*}[t]
\caption{Information ownership and frozen feedback economics.  Coalition
results hold for each of the ten fixed payloads.  Economic statistics cover
only O1 points within the measured periodic performance domain.}
\label{tab:economics}
\centering
\scriptsize
...
\midrule
Fixed-action supplementary dimension & $\rank(L_\perp)$ & 1 for 10/10 & One scalar is algebraically sufficient. \\
Individual receiver / sender--receiver & row-space test & 0/10 / 0/10 & The scalar is not locally owned by the natural endpoint pair. \\
Minimum coalition & exhaustive $C_{\mathcal S}$ test & 5 agents for 10/10 & In this N=5 topology all agent maps are required to synthesize it. \\
\midrule
...
implemented graph, gains, and observation maps.  Numerical rank uses a fixed
tolerance and does not replace a symbolic proof for arbitrary parameters.

The sign result is local to an information-compatible operating set.  A fully
specified stochastic prior and nonlinear Bayesian estimator may produce a
useful conditional expected value; that is a different information model and
is not ruled out.  Randomized policies are likewise outside the deterministic
exact-sign statement, although randomization cannot make the hidden exact
sign observable.
...
Functional observability and target-sensor selection are established fields.
Our exact row-space and coalition conditions must be read as an action-value
specialization, not a claim of discovering those linear-algebra principles.
The intended contribution is the control-communication synthesis: exact
action functional, sign decision, minimum statistic versus acquisition cost,
and implementation-faithful reachable closure.
...
an exact identifiability test; its hidden projection quantifies local
value-sign ambiguity and the minimum dimension of supplementary linear
information.  The acquisition problem is separate: a one-scalar deficit may
be distributed across a large coalition and may consume the Pareto headroom
it is meant to exploit.
...
The result motivates information-structure and communication-cost audits
before a value-based scheduler is engineered.  Future work may study
structured priors, nonlinear functional observability, and sparse protocols
...
The theorem validation entry point is
\texttt{experiments/tcns\_information\_limits\_validation.m}; its unit test is
\texttt{tests/test\_tcns\_information\_limits.m}.  Figure generation is
\texttt{paper/tcns/manuscript/make\_gm\_figures.m}.  The authoritative GM
theorem-validation run is
\texttt{results/tcns\_information\_limits\_validation/2026-09-08\_060901}.
