...
\begin{equation}
\xi^a=\col\!\left(e^a,hw^a,p_L^a,hv_L^a,h^2a_L^a,
\{\hat p_{ij}^a,h\hat v_{ij}^a\},
\{\hat p_{iL}^a,h\hat v_{iL}^a,h^2\hat a_{iL}^a\}\right).
\label{eq:aug-state}
\end{equation}
...
the time-L value in x0 is \((A_0^L)^\top\ell_a\), while the stacked history's
...
action, regret over positive values is maximized at q+ and decreases with p;
regret over negative values is maximized at q- and increases with p.  Equating
\((1-p)q_+=p(-q_-)\) yields \eqref{eq:random-regret}.  The deterministic
choices p=0 and p=1 incur worst regrets q+ and -q-, respectively, proving
\eqref{eq:det-regret}.
\end{proof}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.96\textwidth]{figures/fig6_reachable_sign_witnesses.pdf}
...
input as \(T_\nu\xi+t_\nu\).  Starting with \(M_0=T_y\), \(b_0=0\), recurse
\begin{align}
...
\texttt{experiments/tcns\_information\_limits\_validation.m}; the R1 depth
audit is \texttt{experiments/tcns\_r1\_theory\_depth\_validation.m}, with
unit test \texttt{tests/test\_tcns\_r1\_theory\_depth.m}.  Figure generation is
\texttt{paper/tcns/manuscript/make\_gm\_figures.m}.  The authoritative GM
theorem-validation run, generated from source commit \texttt{15177a8}, is
\texttt{results/tcns\_information\_limits\_validation/2026-09-08\_095209}.
