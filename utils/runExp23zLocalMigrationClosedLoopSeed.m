...
for k=1:numel(R.arms)
    ...
    emergencyEnabled=false; emergencySlot=NaN;
    emergencyOpportunities=NaN; emergencyExpectedSuccess=NaN;
    emergencyExpectedErasure=NaN; emergencyExpectedCollision=NaN;
    claimBackoffEnabled=NaN; claimDenseRetryFrames=NaN;
    claimMaxBackoffFrames=NaN; claimRetryAttemptLimit=NaN;
    claimOpportunityCount=NaN; claimScheduleHash=NaN;
    claimAttempts=NaN; lockProofAttempts=NaN; responseAttempts=NaN;
    retryBudgetExhausted=NaN;
    retryEntryFailureBound=NaN; retryUnionFailureBound=NaN;
    if strcmp(arm.kind,'periodic')
        ...
        if armLogical(arm,'claimBackoffEnabled',false)
            C.claimBackoffEnabled=true;
            C.claimDenseRetryFrames=armNumeric(arm, ...
                'claimDenseRetryFrames',R.claimDenseRetryFrames);
            C.claimMaxBackoffFrames=armNumeric(arm, ...
                'claimMaxBackoffFrames',R.claimMaxBackoffFrames);
            C.claimRetryAttemptLimit=armNumeric(arm, ...
        ...
        if isfield(arm,'emergencyFallback') && arm.emergencyFallback
            Q=addLocalMigrationEmergencyData( ...
                Q,geometry.migration,C,R.emergencySlot);
            emergencyEnabled=true;
            emergencySlot=R.emergencySlot;
        end
        ...
        if emergencyEnabled
            ...
            emergencyOpportunities=nnz(emergency);
            emergencyExpectedSuccess=nnz(Q.dataSuccessMask(emergency,:));
            emergencyExpectedErasure=nnz(Q.dataErasureMask(emergency,:));
            emergencyExpectedCollision=nnz(Q.dataCollisionMask(emergency,:));
        end
        ...
        claimBackoffEnabled=K.claimBackoffEnabled;
        claimDenseRetryFrames=K.claimDenseRetryFrames;
        claimMaxBackoffFrames=K.claimMaxBackoffFrames;
        claimRetryAttemptLimit=K.claimRetryAttemptLimit;
        claimOpportunityCount=K.claimOpportunityCount;
        claimScheduleHash=K.claimScheduleHashExact;
        claimAttempts=K.claimAttempts;
        lockProofAttempts=K.lockProofAttempts;
        responseAttempts=K.responseAttempts;
        retryBudgetExhausted=K.retryBudgetExhausted;
        retryCertificate=localUnionMigrationRetryCertificate(C, ...
            geometry.migration.incidentUnionEdgeCount);
        retryEntryFailureBound=retryCertificate.entryFailureUpperBound;
        retryUnionFailureBound=retryCertificate.unionFailureUpperBound;
    end
    ...
    row.emergencyEnabled=double(emergencyEnabled);
    row.emergencySlot=emergencySlot;
    row.emergencyOpportunities=emergencyOpportunities;
    row.emergencyExpectedSuccess=emergencyExpectedSuccess;
    row.emergencyExpectedErasure=emergencyExpectedErasure;
    row.emergencyExpectedCollision=emergencyExpectedCollision;
    row.claimBackoffEnabled=claimBackoffEnabled;
    row.claimDenseRetryFrames=claimDenseRetryFrames;
    row.claimMaxBackoffFrames=claimMaxBackoffFrames;
    row.claimRetryAttemptLimit=claimRetryAttemptLimit;
    row.claimOpportunityCount=claimOpportunityCount;
    row.claimScheduleHash=claimScheduleHash;
    row.claimAttempts=claimAttempts;
    row.lockProofAttempts=lockProofAttempts;
    row.responseAttempts=responseAttempts;
    row.retryBudgetExhausted=retryBudgetExhausted;
    row.retryEntryFailureBound=retryEntryFailureBound;
    row.retryUnionFailureBound=retryUnionFailureBound;
    rows(k)=row;
end
...
function value=armLogical(arm,name,defaultValue)

value=defaultValue;
if isfield(arm,name), value=arm.(name); end
if ~isscalar(value) || ~(islogical(value) || ...
        (isnumeric(value) && isfinite(value) && any(value==[0 1])))
    error('runExp23zLocalMigrationClosedLoopSeed: invalid arm.%s.',name);
end
value=logical(value);
...
function value=armNumeric(arm,name,defaultValue)
