function exp14a_holdout_primary(resumeDir)
%EXP14A_HOLDOUT_PRIMARY Frozen 100-seed 6-DOF primary validation.
%
% An optional existing run directory resumes only complete seed/scenario
% groups from tidy_checkpoint.csv.  This recovery path was added after the
% interactive runner was externally interrupted at 5600/10500 rows.  It does
% not alter the frozen registry, configurations, group ordering or outputs.
...
if isempty(resumeDir)
    ...
    [allRows,completed] = loadCheckpoint( ...
        checkpoint,groupSeed,groupScenario,R);
    fprintf(['EXP14A RESUME: %d complete groups and %d rows recovered; ' ...
...
for first = 1:batchSize:numel(pending)
    ...
    fprintf('  completed pending groups %3d--%3d / %3d; rows %5d / %5d\n', ...
        indices(1),indices(end),nGroup,numel(allRows),R.expected.primaryRuns);
end
...
if ~isfolder(resumeDir)
    error('exp14a_holdout_primary: resume directory does not exist.');
end
openedPath = fullfile(resumeDir,'holdout_opened.json');
checkpointPath = fullfile(resumeDir,'tidy_checkpoint.csv');
if ~isfile(openedPath) || ~isfile(checkpointPath)
    error(['exp14a_holdout_primary: resume requires holdout_opened.json ' ...
        'and tidy_checkpoint.csv.']);
end
opened = jsondecode(fileread(openedPath));
if opened.registryHash~=registryHash || opened.policyParametersMayChange
    error('exp14a_holdout_primary: opened-run registry contract mismatch.');
end

[expRoot,runId] = fileparts(resumeDir);
expRun.name = 'exp14a_holdout_primary';
expRun.runId = runId;
...
expRun.meta.sourceFile=which('exp14a_holdout_primary');
expRun.t0=tic;
diary(expRun.logFile);
diary on;
fprintf('\n============================================================\n');
fprintf('RESUME: %s\n',expRun.name);
fprintf('ID    : %s\n',runId);
fprintf('WHEN  : %s\n',expRun.meta.resumedAt);
fprintf('CAUSE : external interactive-run interruption\n');
fprintf('CHANGE: recovery mechanism only; frozen scientific design unchanged\n');
fprintf('============================================================\n\n');
...
if ~isequal(T.Properties.VariableNames(:),expectedFields)
    error('exp14a_holdout_primary: checkpoint schema differs from frozen row.');
end
if height(T)>R.expected.primaryRuns || mod(height(T),35)~=0
    error('exp14a_holdout_primary: checkpoint row count is not group-complete.');
end

completed=false(numel(groupSeed),1);
matched=false(height(T),1);
for g=1:numel(groupSeed)
    sc=R.scenarios(groupScenario(g));
    idx=T.seed==groupSeed(g) & T.scenario==string(sc.id);
    n=nnz(idx);
    if n==0, continue; end
    if n~=35
        error('exp14a_holdout_primary: checkpoint group %d has %d/35 rows.',g,n);
    end
    keys=T.stage(idx)+"|"+T.family(idx)+"|"+T.arm(idx);
    if numel(unique(keys))~=35 || ...
            ~isscalar(unique(T.TRACE_HASH_EXACT(idx))) || ...
            ~isscalar(unique(T.CHANNEL_STATE_HASH(idx)))
        error('exp14a_holdout_primary: checkpoint group %d failed CRN/key audit.',g);
    end
    completed(g)=true;
    matched=matched|idx;
end
if ~all(matched) || nnz(completed)*35~=height(T)
    error('exp14a_holdout_primary: checkpoint contains undeclared rows.');
end
rows=table2struct(T);
...
finiteRows = isfinite(T.RMSE) & isfinite(T.MINSEP) & ...
    ...
    isfinite(T.ROLL_PEAK_DEG) & isfinite(T.SATURATION);
...
[names,passed,detail] = addGate(names,passed,detail,'physical_accounting', ...
    ...
    'plus busy-time identities close']);
...
[names,passed,detail] = addGate(names,passed,detail,'divergence_accounting', ...
    divergenceAccounted,sprintf('%d diverged runs retained as safety failures', ...
    sum(T.DIVERGED)));
