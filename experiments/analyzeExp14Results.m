...
[oodContrasts,oodSafety]=secondaryOODContrasts(O,R);
writetable(oodContrasts,fullfile(oodDir,'secondary_ood_paired_contrasts.csv'));
writetable(oodSafety,fullfile(oodDir,'secondary_ood_safety_intervals.csv'));
...
audit.secondaryOODContrasts=size(oodContrasts,1);
audit.secondaryOODMultiplicity='DESCRIPTIVE_UNADJUSTED';
...
S.meanP95TrueAoI=splitapply(@nanmeanLocal,T.P95_TRUE_AOI,G);
S.meanP99TrueAoI=splitapply(@nanmeanLocal,T.P99_TRUE_AOI,G);
...
function [C,Safety]=secondaryOODContrasts(O,R)

defs={ ...
    'hybrid-vs-p10','proposed-hybrid','periodic-p10'; ...
    'hybrid-vs-p20','proposed-hybrid','periodic-p20'; ...
    'hybrid-vs-state','proposed-hybrid','state-default'; ...
    'hybrid-vs-belief','proposed-hybrid','belief-default'; ...
    'piggyback-vs-hybrid','proposed-piggyback','proposed-hybrid'};
metrics={'RMSE','OFFERED_UTIL','MEAN_TRUE_AOI', ...
    'P95_TRUE_AOI','P99_TRUE_AOI'};
C=table(); Safety=table();
for point=unique(O.family,'stable')'
    for k=1:size(defs,1)
        name=string(defs{k,1});
        armA=string(defs{k,2}); armB=string(defs{k,3});
        [A,B]=pairBySeed(selectOOD(O,point,armA), ...
            selectOOD(O,point,armB),R.seeds);
        for q=1:numel(metrics)
            metric=metrics{q};
            t=pairedCI(A.(metric),B.(metric),numel(R.seeds));
            b=pairedBootstrapCI(A.(metric),B.(metric), ...
                R.bootstrapReplicates,R.bootstrapSeed,numel(R.seeds));
            row=table(point,name,armA,armB,string(metric), ...
                mean(A.(metric),'omitnan'),mean(B.(metric),'omitnan'), ...
                t.meanD,t.stdD,t.lo,t.hi,b.lo,b.hi,t.nRequested, ...
                t.nPairs,t.nDropped,double(t.complete), ...
                'VariableNames',{'oodPoint','contrast','armA','armB', ...
                'metric','meanA','meanB','meanDifferenceAminusB', ...
                'stdDifference','pairedTLo','pairedTHi','bootstrapLo', ...
                'bootstrapHi','nRequested','nPairs','nDropped','complete'});
            C=[C; row]; %#ok<AGROW>
        end
        wa=wilsonCI(sum(A.SAFEFAIL),height(A));
        wb=wilsonCI(sum(B.SAFEFAIL),height(B));
        row=table(point,name,armA,armB,sum(A.SAFEFAIL),height(A), ...
            wa.rate,wa.lo,wa.hi,sum(B.SAFEFAIL),height(B),wb.rate, ...
            wb.lo,wb.hi,'VariableNames',{'oodPoint','contrast','armA', ...
            'armB','failuresA','totalA','rateA','loA','hiA', ...
            'failuresB','totalB','rateB','loB','hiB'});
        Safety=[Safety; row]; %#ok<AGROW>
    end
end
...
function T=selectOOD(O,point,arm)

T=O(O.family==point & O.arm==arm,:);
...
if height(A)~=numel(seeds) || height(B)~=numel(seeds) || ...
    ...
    error('analyzeExp14Results: incomplete or non-CRN paired contrast.');
end
