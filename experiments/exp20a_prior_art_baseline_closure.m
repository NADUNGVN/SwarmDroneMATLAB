...
if isempty(resumeDir)
    ...
    recordTechnicalAmendment(expRun.dir);
end
...
function recordTechnicalAmendment(target)

root=projectRoot();
amendmentDir=fullfile(target,'technical_amendment_source');
if ~isfolder(amendmentDir), mkdir(amendmentDir); end
files={'docs/EXP20A_TECHNICAL_AMENDMENT_2026-09-01.md', ...
    'utils/applyExp20AArm.m', ...
    'tests/test_exp20a_formation_contracts.m', ...
    'experiments/exp20a_prior_art_baseline_closure.m'};
for k=1:numel(files)
    name=replace(files{k},{'/' '\'},'__');
    copyfile(fullfile(root,files{k}),fullfile(amendmentDir,name));
end
record=struct('recordedAt',char(datetime('now', ...
    'Format','yyyy-MM-dd HH:mm:ss')), ...
    'classification','execution-only dispatch correction', ...
    'scientificRegistryChanged',false, ...
    'seedsChanged',false,'armsChanged',false, ...
    'parametersChanged',false,'metricsChanged',false, ...
    'nativeRowsAlreadyComplete',1800, ...
    'formationRowsProducedBeforeFailure',0, ...
    'correction',['map the two frozen EXP20A oracle identifiers to ' ...
    'their semantically identical EXP19A adapter identifiers']);
writeJson(fullfile(target,'technical_amendment_2026-09-01.json'),record);
