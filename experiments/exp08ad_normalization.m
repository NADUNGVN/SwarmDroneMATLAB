...
% EXP08A values at Stressed / sparse6 / N=20, from its THREE-seed pass.
%
% The seed count has to match. The published EXP08A figures are the
% 20-seed means (0.4317 and 0.3541); comparing a 3-seed run against
% those passes only because they happen to be close, which is a much
% weaker check than it appears. Against the 3-seed values the match is
% exact, and that is what makes this a real additivity test.
expP10  = 0.4316;
expCaus = 0.3539;
...
% Tolerance matches the precision of the reference itself. The
% constants above are 4-decimal readings, so a tighter bound would be
% false rigour: it would be testing rounding, not additivity.
if abs(expP10-gotP10) < 1e-4 && abs(expCaus-gotCaus) < 1e-4
