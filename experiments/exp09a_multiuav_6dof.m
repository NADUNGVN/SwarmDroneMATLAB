...
expRun = startExperiment('exp09a_multiuav_6dof');
    ...
    for iX = 1:nMode
        for iM = 1:nMethod
            ...
            parfor s = 1:numSeeds
                ...
                cfg.net.seed = 3900000 + 10000*iS + s;
...
fprintf('\n');
if nDefer > 0
    fprintf('  EXP09A GATES: %d PASS, %d FAIL, %d DEFERRED to the 20-seed run\n', ...
        sum(strcmp(gateStatus,'PASS')), nFailed, nDefer);
elseif nFailed == 0
    fprintf('  EXP09A GATES: ALL PASS (%d/%d)\n', numel(gateNames), numel(gateNames));
else
    fprintf('  EXP09A GATES: %d of %d FAILED\n', nFailed, numel(gateNames));
