...
discoveryStatusAttempts=0;
requestStatusAttempts=0;
grantWithoutDecodedRequest=0;
...
for frame=1:F
    ...
    discovery=mod(frame-1,C.statusPeriodFrames)==0;
    statusRequest=ready & (refreshNeeded | forceStatusRequest);
    statusRequest(tupleChangePending)=false;
    statusTx=ready & (discovery | statusRequest);
    statusTx(tupleChangePending)=false;
    forceStatusRequest(statusTx)=false;
    requestStatusAttempts=requestStatusAttempts+nnz( ...
        statusTx & statusRequest);
    discoveryStatusAttempts=discoveryStatusAttempts+nnz( ...
        statusTx & ~statusRequest);
    ...
    for owner=1:N
        ...
        decodedRequest=Astatus.successMask(owner,:) & ...
            reshape(statusRequest,1,[]);
        clients=find(decodedRequest & ...
    ...
    grantWithoutDecodedRequest=grantWithoutDecodedRequest+nnz( ...
        grantTx & ~any(pending,2));
    if C.deterministicControlSlots
        grantChoice=(1:N)';
    else
    ...
    debugStatusRequest(frame,:)=statusRequest;
...
out.discoveryStatusAttempts=discoveryStatusAttempts;
out.requestStatusAttempts=requestStatusAttempts;
out.grantWithoutDecodedRequest=grantWithoutDecodedRequest;
...
fallbackPerFrameNode=squeeze(sum(debugFallbackTx,2));
out.maxFallbackAttemptsPerNodeFrame=max(fallbackPerFrameNode(:));
...
out.debug=struct('ready',debugReady,'active',debugActive, ...
    ...
    'statusRequest',debugStatusRequest, ...
