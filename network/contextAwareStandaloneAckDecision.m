...
freshnessHeadroom=zeros(nEntries,1);
...
for k=1:nEntries
    ...
    if policy.freshnessHeadroomEnabled
        freshnessHeadroom(k)=max(0,1-sampleAge/ ...
            cfg.aoiEvent.aoiThreshold);
    else
        freshnessHeadroom(k)=1;
    end
    successEstimate(k)=mapValue(policy.ackSuccessEstimate,target,node,net.N);
    benefitTerms(k)=successEstimate(k)*dataAirtime* ...
        redundancyScore(k)*freshnessHeadroom(k);
end
...
info=struct('allow',allow,'forced',forced,'reason',reason, ...
    'rule','feasibility-value','contextDecision',true, ...
    'evaluated',evaluated, ...
    ...
    'freshnessHeadroomMean',mean(freshnessHeadroom), ...
