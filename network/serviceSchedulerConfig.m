...
if any(strcmp(S.mode,{'continuous-dstr-replay','continuous-elcs-replay'}))
    validateDstrSchedule(S.dstrSchedule,cfg);
    if ~S.continuousExactAirtime
        error(['serviceSchedulerConfig: continuous schedule replay requires ' ...
            'continuousExactAirtime=true.']);
    end
    if cfg.mac.backgroundLoad>0
        if ~isfield(S.dstrSchedule,'backgroundOverlay') || ...
                ~isstruct(S.dstrSchedule.backgroundOverlay) || ...
                ~isfield(S.dstrSchedule.backgroundOverlay,'version') || ...
                ~strcmp(S.dstrSchedule.backgroundOverlay.version, ...
                    'ELCS-W-SHARED-BACKGROUND-OVERLAY-v1') || ...
                ~isfield(S.dstrSchedule.backgroundOverlay,'backgroundLoad') || ...
                abs(S.dstrSchedule.backgroundOverlay.backgroundLoad- ...
                    cfg.mac.backgroundLoad)>1e-12
            error(['serviceSchedulerConfig: nonzero replay background ' ...
                'requires a matching ELCS-W physical-overlay certificate.']);
        end
    end
end
...
if isfield(Q,'controlAirtimeSec') && ...
        (~isscalar(Q.controlAirtimeSec) || ...
        ~isfinite(Q.controlAirtimeSec) || Q.controlAirtimeSec<=0)
    error(['serviceSchedulerConfig: continuous replay control airtime ' ...
...
if isfield(Q,'controlGroupBytes')&& ...
        (numel(Q.controlGroupBytes)~=n|| ...
        any(~isfinite(Q.controlGroupBytes)|Q.controlGroupBytes<0| ...
        Q.controlGroupBytes~=floor(Q.controlGroupBytes)))
    error('serviceSchedulerConfig: malformed D-STR management bytes.');
end
if isfield(Q,'controlPhyRateBps')&& ...
        (~isscalar(Q.controlPhyRateBps)|| ...
        ~isfinite(Q.controlPhyRateBps)||Q.controlPhyRateBps<=0)
    error('serviceSchedulerConfig: malformed D-STR management PHY rate.');
end
if isfield(Q,'controlGroupEndTime') && ...
        (numel(Q.controlGroupEndTime)~=n || any( ...
        Q.controlGroupEndTime<Q.controlGroupStartTime))
    error('serviceSchedulerConfig: malformed D-STR control intervals.');
end
if isfield(Q,'dataGroup') && numel(Q.dataGroup)~=numel(Q.dataStartTime)
    error('serviceSchedulerConfig: malformed D-STR DATA groups.');
end
